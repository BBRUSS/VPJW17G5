...
void ImgTask::run()
{
    ...
    std::vector< std::vector<cv::Point2f> > markerCorners;          //Vector contains the Cornerposition of detected Marker in Order by Vector Marker Corners
    ...
    std::vector< std::vector<cv::Point2f> > rejectedCandidates;     //Contains the Position of rejectedCandidates
    ...
    {
        std::vector<cv::Point2f> tempVec, tempVec2;
        ...
        cv::undistortPoints(tempVec, tempVec2, cameraMatrix, distCoeffs, cv::noArray(), cameraMatrix);

        // Apply perspective Transformation
        cv::perspectiveTransform(tempVec2, tempVec, perspTransfMatrix);     //Umrechnung auf das große Feld!
...
void ImgTask::setArucoParameters(cv::Ptr<cv::aruco::DetectorParameters> arucoParameters)
{
...
{
    return Pointlist::fromVector(QVector<cv::Point2f>::fromStdVector(vec));
}

std::vector<cv::Point2f> ImgTask::PointlistToStdVector(Pointlist vec)
{
    return vec.toVector().toStdVector();
}

cv::Point2f ImgTask::calculatemiddlepoint(Pointlist marker1, double offset, double angle)
{
    // VARIABLES
    double angleRAD;                                //Angle in Radiant
    cv::Point2f temp1 = cv::Point2f(0,0);                   //Temporary
    cv::Point2f middlepointMarker1 = cv::Point2f(0,0);      //Vector with calculated Middlepoint for Marker 1
...
///Calculate the Middlepoint (x,y) between two Marker, based on the Corner of booth Marker
{
    // VARIABLES
    cv::Point2f temp1 = cv::Point2f(0,0);                   //Temporary
    cv::Point2f temp2 = cv::Point2f(0,0);                   //Temporary
    cv::Point2f middlepointMarker1 = cv::Point2f(0,0);      //calculated Middlepoint for Marker 1
    cv::Point2f middlepointMarker2 = cv::Point2f(0,0);      //calculated Middlepoint for Marker 2
    cv::Point2f middlepoint = cv::Point2f(0,0);             //Vector with calculated Middlepoint for Robot
    ...
    Pointinformations.append(cv::Point2f(middlepoint.x-middlepointMarker1.x,middlepoint.y-middlepointMarker1.y));
    Pointinformations.append(cv::Point2f(middlepoint.x-middlepointMarker2.x,middlepoint.y-middlepointMarker2.y));
