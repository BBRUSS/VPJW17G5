...
bool ArucoSerializer::drawArucoDictionarySingle(cv::Ptr<cv::aruco::Dictionary> Dict, QString Path, QString NamePrefix, int Size, int ID) {
    try {
        cv::Mat Marker;
        cv::aruco::Dictionary* DictObject;
        DictObject = Dict.get();
        cv::aruco::drawMarker(Dict, ID, Size, Marker);
